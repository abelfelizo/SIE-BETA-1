...
function winner(votes){
  ...
  const w = (arr[0] ? arr[0][0] : null) || null;
  const wv = Number((arr[0] ? arr[0][1] : null)||0);
  const rv = Number((arr[1] ? arr[1][1] : null)||0);
...
export function computePresRisk(nationalVotes){
  ...
  const topShare = tot ? Number((arr[0] ? arr[0][1] : null)||0)/tot : 0;
  const margin = tot ? (Number((arr[0] ? arr[0][1] : null)||0)-Number((arr[1] ? arr[1][1] : null)||0))/tot : 0;
...
export function computeDiputadosCurules(dipLevel, curulesMap){
  ...
  const out = {};
  for(const prov of Object.keys((dipLevel ? dipLevel.territoriosProv : null)||(dipLevel ? dipLevel.territorios : null)||{})){
    const t = (dipLevel.territoriosProv||dipLevel.territorios)[prov];
    const seats = Number((curulesMap ? curulesMap[prov] : null) || 0);
    if(!seats) continue;
...
export function applyPartyAdjust(votes, adjustPP){
  ...
  for(const p of Object.keys(baseShares)){
    shares[p] = Math.max(0, baseShares[p] + (Number((adjustPP ? adjustPP[p] : null)||0)/100));
  }
...
export function applyMobilizacion(meta, votes, movilizacion){
  ...
  const inscritos = Number((meta ? meta.inscritos : null)||0);
  const emitidos = Number((meta ? meta.emitidos : null)||0);
  const abst = Math.max(0, inscritos-emitidos);
  const cap = Math.round(abst*0.60);
  if(!cap || !inscritos) return {meta:{...meta}, votes:{...votes}};

  let add = 0;
  if((movilizacion ? movilizacion.mode : null) === "pp"){
    const pp = Number((movilizacion ? movilizacion.value : null)||0);
    add = Math.round(inscritos*(pp/100));
  }else{ // captura
    const pct = Number((movilizacion ? movilizacion.value : null)||0);
  ...
  return {
    meta: {...meta, emitidos: emitidos+add, validos: (Number((meta ? meta.validos : null)||emitidos)+add)},
    votes: outVotes
  };
}
